# planning_service/app.py  —  v2
# Améliorations : pagination sur /etudiants + gunicorn (plus de debug=True)
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s [%(levelname)s] planning_service — %(message)s'
)
...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get(
    'DATABASE_URL', 'postgresql://planning_user:planning_pass@localhost:5432/planning_db'
)
...
def health():
    return jsonify({'status': 'ok', 'service': 'planning'}), 200
...
def get_etudiants(user):
    """
    AMÉLIORATION 3 — Pagination
    Paramètres query string :
      - promotion_id : filtre par promo (optionnel)
      - page         : numéro de page (défaut 1)
      - per_page     : taille de page (défaut 50, max 200)
      - all          : si "true", retourne tout sans pagination
                       (utilisé par emargement_service pour créer les présences)
    """
    ...
    # Mode sans pagination : uniquement pour appels internes inter-services
    if request.args.get('all') == 'true':
        etudiants = q.order_by(Etudiant.nom).all()
        return jsonify({
    ...
    per_page = min(request.args.get('per_page', 50, type=int), 200)  # max 200 par page

    pagination = q.order_by(Etudiant.nom).paginate(
        page=page, per_page=per_page, error_out=False
    )

    return jsonify({
        'etudiants': [e.to_dict() for e in pagination.items],
...
def ajouter_etudiant(user):
    ...
    db.session.add(e)
    db.session.commit()
    return jsonify({'message': 'étudiant ajouté', 'etudiant': e.to_dict()}), 201
...
def _importer_depuis_excel(fichier, promotion_id):
    ...
    for i, row in enumerate(lignes):
        ...
        if len(cols) < 3:
            erreurs.append(f'ligne {i+1} ignorée (pas assez de colonnes)')
            nb_ignores += 1
            continue
        ...
        if not nom or not prenom or not email or '@' not in email:
            erreurs.append(f'ligne {i+1} ignorée (données invalides)')
            nb_ignores += 1
            continue

        if Etudiant.query.filter_by(email=email).first():
            erreurs.append(f'ligne {i+1} ignorée (email déjà existant) : {email}')
            nb_ignores += 1
            continue
    ...
    db.session.commit()
    logger.info('%d étudiants importés (Excel) dans la promo %d', nb_ajoutes, promotion_id)
    return jsonify({
        'message':    f'{nb_ajoutes} étudiants importés, {nb_ignores} ignorés',
...
def _importer_depuis_lignes_csv(lignes, promotion_id):
    ...
    for i, ligne in enumerate(lignes):
        ...
        cols = ligne.split(';') if ';' in ligne else ligne.split(',')
        cols = [c.strip().strip('"') for c in cols]

        if len(cols) < 3:
            erreurs.append(f'ligne {i+1} ignorée (pas assez de colonnes) : {ligne}')
            nb_ignores += 1
            continue
    ...
    db.session.commit()
    logger.info('%d étudiants importés (CSV) dans la promo %d', nb_ajoutes, promotion_id)
    return jsonify({
        'message':    f'{nb_ajoutes} étudiants importés, {nb_ignores} ignorés',
        'nb_ajoutes': nb_ajoutes,
        'nb_ignores': nb_ignores,
...
def modifier_etudiant(user, id):
    ...
    db.session.commit()
    return jsonify({'message': 'étudiant modifié', 'etudiant': e.to_dict()}), 200


# ── init DB ───────────────────────────────────────────────────────────────────
with app.app_context():
    db.create_all()
    logger.info('Tables planning créées / vérifiées')

# AMÉLIORATION 5 : démarrage via gunicorn — voir Dockerfile
# CMD ["gunicorn", "--bind", "0.0.0.0:5002", "--workers", "2", "--timeout", "60", "--access-logfile", "-", "app:app"]
